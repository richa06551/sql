...
USE feedassignment;
CREATE TABLE Feed1 ( 
name VARCHAR(50), city VARCHAR(50), dept VARCHAR(50), manager VARCHAR(50), shift VARCHAR(20), 
remarks VARCHAR(100), status VARCHAR(20), level VARCHAR(10), location VARCHAR(50), country VARCHAR(50)
);
CREATE TABLE Feed2 ( 
name VARCHAR(50), city VARCHAR(50), dept VARCHAR(50), manager VARCHAR(50), shift VARCHAR(20), 
remarks VARCHAR(100), status VARCHAR(20), level VARCHAR(10), location VARCHAR(50), country VARCHAR(50), 
doj DATE, salary INT, bonus INT, project VARCHAR(100), probation_status VARCHAR(50)
);
CREATE TABLE Feed3 (
    name VARCHAR(50),
    city VARCHAR(50),
...
FROM Feed3
GROUP BY
name, city, dept, manager, shift, remarks, status, level, location, country, doj, salary, bonus, project, 
probation_status, grade, email, mobile, emp_code, team HAVING COUNT(*) > 1;
SELECT *
FROM Feed1
GROUP BY name, city, dept, manager, shift, remarks, status, level, location, country
HAVING COUNT(*) > 1
INTO OUTFILE 'C:/ProgramData/MySQL/MySQL Server 8.4/Uploads/Feed1_Duplicates.csv.csv'
FIELDS TERMINATED BY ',' ENCLOSED BY '"'
LINES TERMINATED BY '\n';
SELECT *
FROM Feed2
GROUP BY name, city, dept, manager, shift, remarks, status, level, location, country, doj, salary, bonus, project, probation_status
HAVING COUNT(*) > 1
INTO OUTFILE 'C:/ProgramData/MySQL/MySQL Server 8.4/Uploads/Feed2_Duplicates_v2.csv'
FIELDS TERMINATED BY ',' ENCLOSED BY '"'
LINES TERMINATED BY '\n';
SELECT *
FROM Feed3
...
HAVING COUNT(*) > 1
INTO OUTFILE 'C:/ProgramData/MySQL/MySQL Server 8.4/Uploads/Feed3_Duplicates_v2.csv'
FIELDS TERMINATED BY ',' ENCLOSED BY '"'
LINES TERMINATED BY '\n';
...
= f1 .shift AND f2.remarks = f1.remarks AND f2.status = f1.status AND f2.level = f1 .level AND 
f2.location = f1 .location AND f2.country = f1.country;
...
CALL InsertData('Feed1', 10);
  CALL InsertData('Feed2', 15);
  CALL InsertData('Feed3', 20);
  SELECT COUNT(*) AS dup_Feed1 FROM (
    SELECT COUNT(*) FROM Feed1 GROUP BY name, city, dept, manager, shift, remarks, status, level, location, country HAVING COUNT(*) > 1
  ) t1;
  SELECT COUNT(*) AS dup_Feed2 FROM (
    SELECT COUNT(*) FROM Feed2 GROUP BY name, city, dept, manager, shift, remarks, status, level, location, country, doj, salary, bonus, project, probation_status HAVING COUNT(*) > 1
  ) t1;
  SELECT COUNT(*) AS dup_Feed3 FROM (
    SELECT COUNT(*) FROM Feed3 GROUP BY name, city, dept, manager, shift, remarks, status, level, location, country, doj, salary, bonus, project, probation_status, grade, email, mobile, emp_code, team HAVING COUNT(*) > 1
  ) t1;
  CREATE TABLE Feed1_Unique AS SELECT DISTINCT * FROM Feed1;
DROP TABLE Feed1;
ALTER TABLE Feed1_Unique RENAME TO Feed1;
...
WHERE (name, city, dept, manager, shift, remarks, status, level, location, country) IN ( 
SELECT name, city, dept, manager, shift, remarks, status, level, location, country FROM Feed3);
SELECT * FROM Feed2_Feed3_Common;
END &&
DELIMITER ;
CALL CompareFeed2Feed3();
DROP DATABASE feedassignment;
